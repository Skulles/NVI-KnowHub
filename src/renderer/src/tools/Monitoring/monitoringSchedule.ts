...
export const MONITORING_RESOURCES_INTERVAL_MS = 5 * 60_000
export const MONITORING_MAX_RESOURCES_BACKOFF_MS = 10 * 60_000
...
export const MONITORING_MAX_RESOURCES_BATCH = 4
...
export function appendLinkStatusSample(
  ...
  const prev = history ?? []
  const next = [...prev, { online, at, replyCount, sentCount }]
  return next.slice(-MONITORING_LINK_STATUS_HISTORY_LIMIT)
}

/** Aggregate packet loss only after the requested number of complete probes. */
export function recentPacketLossPercent(
  history: LinkStatusSample[] | undefined,
  checkCount = 3
): number | null {
  if (!history || history.length < checkCount) return null
  const samples = history.slice(-checkCount)
  if (samples.some((sample) => !sample.sentCount || sample.replyCount === undefined)) return null
  const sent = samples.reduce((sum, sample) => sum + (sample.sentCount ?? 0), 0)
  const replies = samples.reduce((sum, sample) => sum + (sample.replyCount ?? 0), 0)
  if (sent <= 0) return null
  return Math.round((Math.max(0, sent - replies) / sent) * 100)
}
...
export function createProbeSchedule(now = Date.now()): ObjectProbeSchedule {
  return {
    nextLinkAt: now,
    nextServerAt: 0,
    nextServerResourcesAt: 0,
    nextPreviewAt: 0,
    nextMegaphoneStatusAt: 0,
    nextDeviceProbeAt: 0,
    linkFailures: 0,
    serverFailures: 0,
    serverResourcesFailures: 0,
    previewFailures: 0,
...
export function resourcesFailureBackoffMs(failures: number): number {
  const exp = Math.min(
    MONITORING_MAX_RESOURCES_BACKOFF_MS,
    MONITORING_RESOURCES_INTERVAL_MS * 2 ** Math.max(0, failures - 1)
  )
  return Math.min(MONITORING_MAX_RESOURCES_BACKOFF_MS, jitter(exp))
}

/** Retry failed metrics sooner on lossy links instead of waiting for the normal refresh interval. */
export function metricFailureDelayMs(failures: number, tier: SignalTier): number {
  const baseMs = tier === 'degraded' || tier === 'poor' ? 30_000 : MONITORING_PREVIEW_INTERVAL_MS
  return failureBackoffMs(failures, baseMs)
}
